...
class Recorder:
    ...
    def record(self):
        ...
        a = input('Please enter the number [2] and press Enter [Start] to record：\n')
        if a == '2':
            begin = time.time()
            logger.debug("start recording")
            self.start()
            b = input('Please enter the number [2] and press Enter [End] to record：\n')
            if b == '2':
    ...
    def run(self):
        ...
        if count == 0:
            ...
            return None
        logger.debug('Start recording')
        print("Please speak, like 'Recliner Command'")
        while i < count:
            mode = 'x'
            self.record()
            # Recording loop
            while True:
                print('[Re-record this item] Please enter [y] and press Enter, [Save this item and continue] Please enter [s] and press Enter：')
                answer = input()
                if answer.upper() == 'Y':
                    mode = 'w'
                    self.record()
            ...
            while True:
                ...
                try:
                    ...
                    logger.warning(f'The filename {filename} already exists and the program will automatically replace it.')
                    num += 1
                else:
                    num += 1
                    print(f'Recordings are saved at: {filename}')
                    yield raw, filename
                    break
    ...
    def get_count(self):
        ...
        count = input('Please enter the number of recorded samples (during the testing phase, it is recommended to enter 1 to record 1 for easy debugging, enter the number 0 to skip): \n')
        try:
            count = int(count)
        except ValueError as e:
            print('Invalid input, skip recording.')
            return 0
        else:
            if count > 1:
                print('You can enter more than 1, which will record multiple voice messages.')
            return count
